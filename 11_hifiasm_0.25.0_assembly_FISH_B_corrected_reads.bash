...
reads=$wkdir/basecalls/FishB_ALL_simplex_corrected.fa.gz

# print a line to the slurm output that says exactly what was done on this run
echo "This is hifiasm version 0.25.0 running on the file $reads and saving the output to the directory $wkdir/$fish/hifiasm_asm$attempt"

# make directory for the assembly & move to it
mkdir -p $wkdir/$fish/hifiasm_asm$attempt
cd $wkdir/$fish/hifiasm_asm$attempt

# run hifiasm on the simplex reads (without the new --ont flag because it requires fastq input) to generate the assembly
hifiasm \
-t 96 \
-o ONTasm \
$reads
